...
Q_vals = [val['Q_array'] for val in hf.load(os.path.join('data','viscoelasticity','sim_params.pkl'))]
...
for i, Q_real in enumerate(Q_vals):
    ...
    dt = 0.0001
    t = np.arange(0 + dt, 1 + dt, dt)  # second time signal
    ...
    if noise:
        force_real += hf.gaussian_white_noise(0.1 * np.max(force_real), force_real.shape)
    for num_arms in range(1, 5):
        ...
        # do the optimization
        res = optimize.dual_annealing(obj.function, bound, maxiter=1000, local_search_options={'method': 'nelder-mead'})
