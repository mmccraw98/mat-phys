...
max_array_length = 50000000  # SETTING
...
# sample info - maxwell model - single arm
x = np.zeros(time.shape)  # position of the model -> position of the elastic spring and the arm springs (SURFACE HEIGHT)
xd = np.zeros(time.shape)  # position of the arm damper
vd = np.zeros(time.shape)  # velocity of the arm damper
...
for iteration in range(num_chunks):
    ...
    while (i := i + 1) < time.size - 1: #@ TODO: make a 'toy' simulation AFM library???? same backend just smaller numbers -> smaller timestep
        ...
        if zt[i + 1] < x[i + 1]:
            x[i + 1] = zt[i + 1]
            f_e = - model_params['Ee'] * x[i + 1]  # calculate the force in the elastic element
            f_arm = - model_params['arms'][0]['E'] * (
                        x[i + 1] - xd[i])  # calculate the force in the arm due to the arm spring
            vd[i + 1] = - f_arm / model_params['arms'][0][
                'T']  # calculate the velocity of the damper due to the force in the arm
            xd[i + 1] = xd[i] + vd[i] * dt  # calculate the new position of the damper
            f_vdw = 0  # - A * R / (6 * a0**2)  # calculate the adhesion force with interatomic spacing
            f_ts[i + 1] = f_e + f_arm + f_vdw  # tip-sample force is the sum of all forces in the model
            # restrict the damper position so that it is not above the surface position
        ...
        # non-contact
            ...
            f_vdw = 0  # - A * R / (6 * (zt[i] - x[i])**2)  # calculate VdW with the separation between the tip and the sample
            f_e = - model_params['Ee'] * x[
                i] - f_vdw  # calculate the force acting on the elastic arm (spring force and vdw)
            f_arm = -f_e  # the elastic spring pulls on the arms, thus force in the arms is equal and opposite to elastic force
            vd[i + 1] = -f_arm / model_params['arms'][0][
                'T']  # calculate the velocity in the damper from the force in the arm
            xd[i + 1] = xd[i] + vd[i + 1] * dt  # calculate the position of the damper
            x[i + 1] = model_params['Ee'] / (model_params['Ee'] + model_params['arms'][0][
                'E']) * xd[i + 1]  # calculate the new position of the sample, derivation in jupyter notebook
            f_ts[i + 1] = f_vdw  # sample and tip are pulled towards each other
